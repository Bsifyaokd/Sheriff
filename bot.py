...
async def safe_send_message(chat_id: int, text: str, reply_markup=None, max_retries=3):
    """Отправляет сообщение с автоматическим повтором при флуд‑контроле."""
    last_exc = None
...
async def try_restore_buttons(duel: DuelSession, msg_id: Optional[int]):
    """Безопасно восстанавливает кнопки, игнорируя ошибки."""
    if not msg_id:
        return
    try:
...
async def send_turn_message_retry(duel: DuelSession, max_retries=5):
    """Отправляет сообщение хода с повторными попытками при флуд‑контроле."""
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.warning(f"Flood control при ходе: ждём {e.retry_after} сек. (попытка {attempt}/{max_retries})")
            await asyncio.sleep(e.retry_after)
            last_exc = e
        except Exception as e:
    ...
    logger.error(f"Не удалось отправить сообщение хода после {max_retries} попыток: {last_exc}")
    try:
...
async def process_action(callback: CallbackQuery):
    try:
        ...
        if duel.processing:
            await callback.answer("Подождите, идёт обработка предыдущего действия.", show_alert=True)
            return
        ...
        # Отправляем результат с защитой от флуда
        try:
            await safe_send_message(duel.chat_id, extra_msg)
        except Exception as e:
            logger.error(f"Не удалось отправить результат действия: {e}")
        ...
        # Передаём ход (без отдельного сообщения «Теперь черёд»)
        duel.current_turn = opponent_id
        # Отправляем новое сообщение хода для противника
        await send_turn_message(duel)
        duel.processing = False
        ...
        if 'duel' in locals():
            try:
                await safe_send_message(duel.chat_id, "⚠️ Дуэль прервана из‑за технической ошибки.")
            except:
                pass
            await cleanup_duel(duel.duel_id)
